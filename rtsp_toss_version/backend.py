from fastapi import FastAPI, HTTPException, BackgroundTasks, File, UploadFile, Form, APIRouter
...
import shutil
from image_size_ocr import process_image_for_web, resize_image_only_for_web, ocr_only_for_web
...
def record_rtsp(location, ip, bitrate=3000):
    ...
    cmd = [
        ...
        '-timeout', '30000000',            # 30초 타임아웃 (30,000,000 마이크로초)
...
def reset_device_endpoint(device: DeviceReset):
    ...
    if location not in RESET_IPS:
        raise HTTPException(status_code=404, detail=f"위치 '{location}'를 찾을 수 없습니다.")
    ...
    try:
        ...
        if success:
            return {"status": "success", "message": f"{LOCATION_DISPLAY_NAMES.get(location, location)} 장치 리셋 성공"}
        else:
            raise HTTPException(status_code=500, detail=f"{LOCATION_DISPLAY_NAMES.get(location, location)} 장치 리셋 실패")
    except Exception as e:
...
async def process_image(
    ...
    if ext not in ['.jpg', '.jpeg', '.png']:
        raise HTTPException(status_code=400, detail="지원하지 않는 파일 형식입니다. JPG 또는 PNG 파일만 가능합니다.")
    ...
    try:
        ...
        if result["success"]:
            ...
            raise HTTPException(status_code=500, detail=result.get("error", "이미지 처리 중 오류가 발생했습니다."))
...
async def resize_image_only(
    ...
    try:
        ...
        if result["success"]:
            ...
            raise HTTPException(status_code=500, detail=result.get("error", "이미지 크기 조정 중 오류가 발생했습니다."))
            
    except Exception as e:
        # 오류 발생 시 임시 파일 정리
        if os.path.exists(temp_file_path):
            os.remove(temp_file_path)
        raise HTTPException(status_code=500, detail=f"이미지 크기 조정 중 오류: {str(e)}")
