...
def log_mpv_keyboard_shortcuts():
    ...
    logger.info(
        "음량 조절: 9(작게 감소), 0(작게 증가), -(크게 감소), =(크게 증가), m 또는 /(음소거)"
    )
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                rtsp_processes.append(process)
                logger.info(
                    f"Intel Mac Homebrew 경로에서 MPV로 RTSP 스트림 시작: {rtsp_url}"
                )
                log_mpv_keyboard_shortcuts()
                return process.pid
            except Exception as e:
                ...
                try:
                    ...
                    try:
                        ...
                        rtsp_processes.append(process)
                        logger.info(
                            f"Homebrew 경로에서 ffplay로 RTSP 스트림 시작: {rtsp_url}"
                        )
                        return process.pid
                    except Exception as e:
                        try:
                            ...
                            rtsp_processes.append(process)
                            logger.info(
                                f"Intel Mac Homebrew 경로에서 ffplay로 RTSP 스트림 시작: {rtsp_url}"
                            )
                            return process.pid
                        except Exception as e:
        ...
        for i, process in enumerate(process_list):
            if process.pid == pid:
                try:
                    ...
                    raise HTTPException(
                        status_code=500, detail=f"프로세스 종료 실패: {str(e)}"
                    )
