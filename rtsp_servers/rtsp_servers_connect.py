...
# 카메라 인증 정보
CAMERA_AUTH = {"username": "admin", "password": "admin1234"}
...
class StreamManager:
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    print(
                        "Mac에서는 'brew install ffmpeg' 명령으로 ffmpeg/ffplay를 설치하세요."
                    )
        except Exception as e:
            print(f"RTSP 스트림 실행 오류: {e}")
    ...
    def record_rtsp(self, location, ip):
        ...
        cmd = [
            ...
            "mp4",
            "-movflags",
            "+faststart+frag_keyframe+empty_moov",  # 안정성 향상 옵션 추가
            "-frag_duration",
            "1000000",  # 프래그먼트 길이 1초
            "-avoid_negative_ts",
            "make_zero",  # 타임스탬프 문제 해결
            "-y",
            mp4_output_file,
        ]
        ...
        try:
            ...
            with open(log_file, "w") as log:
                try:
                    ...
                    mp4_transcode_file = os.path.join(
                        recordings_dir, f"{location}_{timestamp}_transcode.mp4"
                    )
                    # 트랜스코딩 명령 재구성
                    cmd = [
                        "ffmpeg",
                        ...
                        "zerolatency",  # 지연 최소화
                        "-profile:v",
                        "baseline",  # 호환성 좋은 프로파일
                        "-level",
                        "4.1",  # 1080p를 지원하는 레벨로 수정
                        "-pix_fmt",
                        "yuv420p",  # 가장 호환성 좋은 픽셀 포맷
                        # 오디오는 AAC (MP4 표준)
                        "-c:a",
                        "aac",
                        "-b:a",
                    ...
                    try:
                        ...
                        ts_output_file = os.path.join(
                            recordings_dir, f"{location}_{timestamp}.ts"
                        )
                        # 복사 모드로 명령 재구성
                        cmd = [
                            "ffmpeg",
            ...
            try:
                ...
                with open(log_file, "w") as log:
                    try:
                        ...
                        try:
                            ...
                            self.is_recording = True
                            print(
                                f"Mac에서 TS 형식으로 RTSP 스트림 녹화 시작: {rtsp_url}"
                            )
                            print(f"녹화 파일: {ts_output_file}")
                            print(f"로그 파일: {log_file}")
                ...
                try:
                    cmd[0] = (
                        "/usr/local/bin/ffmpeg"  # Intel Mac에서 Homebrew로 설치한 경우
                    )
                    with open(log_file, "w") as log:
                        try:
                            process = Popen(cmd, stdout=log, stderr=log)
                            ...
                            try:
                                ...
                                self.is_recording = True
                                print(
                                    f"Intel Mac에서 TS 형식으로 RTSP 스트림 녹화 시작: {rtsp_url}"
                                )
                                print(f"녹화 파일: {ts_output_file}")
                                print(f"로그 파일: {log_file}")
                                ...
                                try:
                                    ...
                                    print(
                                        f"Intel Mac에서 모든 스트림 복사 방식 실패: {e}"
                                    )
                                    return False
                except Exception as e:
                    print(f"RTSP 스트림 녹화 오류: {e}")
                    print(
                        "Mac에서는 'brew install ffmpeg' 명령으로 ffmpeg를 설치하세요."
                    )
                    return False
        except Exception as e:
            print(f"RTSP 스트림 녹화 오류: {e}")
    ...
    def play_ffplay_monitor(self, rtsp_url, location):
        ...
        try:
            process = Popen(cmd)
            self.processes_rtsp.append(
                process
            )  # 모니터링용 프로세스도 rtsp 목록에 추가
            ...
            try:
                ...
                try:
                    ...
                    self.processes_rtsp.append(process)
                    print(
                        f"Intel Mac Homebrew 경로에서 녹화 모니터링 ffplay 시작: {rtsp_url}"
                    )
                except Exception as e:
    ...
    def play_rtmp(self, location, ip, stream_key):
        ...
        thread = threading.Thread(
            target=self._background_rtmp_task, args=(location, ip, stream_key)
        )
        thread.daemon = True
        thread.start()
...
class DeviceController:
    ...
    def _toggle_http_camera(self, ip, username, password, power_on):
        ...
        try:
            ...
            if response.status_code == 200:
                messagebox.showinfo(
                    "성공", f"HTTP 카메라 {ip} 전원 {'켜짐' if power_on else '꺼짐'}"
                )
                return True
...
class ApplicationGUI:
    ...
    def _create_rtsp_tab(self, notebook):
        ...
        button_41 = tk.Button(
            ...
            width=20,
            height=2,
            command=lambda: self._update_selected_ip(
                "192.168.116.41", button_41, button_42
            ),
        )
        ...
        button_42 = tk.Button(
            ...
            height=2,
            command=lambda: self._update_selected_ip(
                "192.168.118.42", button_41, button_42
            ),
        )
        ...
        self.location_label = tk.Label(
            rtsp_frame, text="선택된 위치: None", font=("Arial", 10)
        )
        self.location_label.pack(pady=10)

        # 스트림 키 입력
        tk.Label(rtsp_frame, text="스트림 키 입력", font=("Arial", 10)).pack(pady=5)
        ...
        stream_button = tk.Button(
            button_frame, text="송출", width=20, height=2, command=self._start_stream
        )
        ...
        self.recording_status = tk.Label(
            record_frame, text="녹화 상태: 중지됨", font=("Arial", 10)
        )
    ...
    def _create_rtmp_status_tab(self, notebook):
        ...
        tk.Label(
            rtmp_frame, text="실행 중인 유튜브 스트림 목록", font=("Arial", 12, "bold")
        ).pack(pady=10)
    ...
    def _create_reset_tab(self, notebook):
        ...
        tk.Label(reset_frame, text="장치 리셋 컨트롤", font=("Arial", 12, "bold")).pack(
            pady=10
        )
    ...
    def _create_camera_tab(self, notebook):
        ...
        tk.Label(camera_frame, text="전경 카메라 제어", font=("Arial", 16)).pack(
            pady=10
        )
        ...
        for name, ip, protocol in CAMERAS:
            ...
            tk.Button(
                ...
                bg="green",
                fg="white",
                command=lambda ip=ip, protocol=protocol: self.device_controller.control_camera(
    ...
    def _start_recording(self):
        """녹화 시작 함수"""
        if (
            not self.stream_manager.selected_location
            or not self.stream_manager.selected_ip
        ):
    ...
    def _update_rtmp_process_list(self):
        ...
        if self.stream_manager.processes_rtmp:
            for idx, process in enumerate(self.stream_manager.processes_rtmp):
                ...
                label = tk.Label(
                    self.process_list_frame, text=process_info, font=("Arial", 10)
                )
