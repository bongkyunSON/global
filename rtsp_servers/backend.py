...
# 스트림 장소 목록
STREAM_LOCATIONS = [
    "1소회의실", "2소회의실", "1세미나실", "2세미나실", "3세미나실",
    "2간담회실", "3간담회실", "4간담회실", "5간담회실", "6간담회실",
    "7간담회실", "8간담회실", "9간담회실", "10간담회실", "11간담회실", "대회의실"
]

# 장치 리셋 IP 매핑
RESET_IPS = {
    "1소회의실": "192.168.100.199", "2소회의실": "192.168.101.199",
    "1세미나실": "192.168.102.199", "2세미나실": "192.168.103.199",
    "3세미나실": "192.168.104.199", "2간담회실": "192.168.105.199",
    "3간담회실": "192.168.106.199", "4간담회실": "192.168.107.199",
    "5간담회실": "192.168.108.199", "6간담회실": "192.168.109.199",
    "7간담회실": "192.168.110.199", "8간담회실": "192.168.111.199",
    "9간담회실": "192.168.112.199", "10간담회실": "192.168.113.199",
    "11간담회실": "192.168.114.199", "대회의실": "192.168.120.199"
}

# 카메라 리스트: 회의장 이름, IP, 제어 방식
CAMERAS = [
    ("1소회의실", "192.168.100.54", "visca"),
    ("2소회의실", "192.168.101.54", "visca"),
    ("1세미나실", "192.168.102.57", "http"),
    ("2세미나실", "192.168.103.57", "visca"),
    ("3세미나실", "192.168.104.54", "visca"),
    ("2간담회실", "192.168.105.57", "http"),
    ("3간담회실", "192.168.106.57", "http"),
    ("4간담회실", "192.168.107.57", "http"),
    ("5간담회실", "192.168.108.57", "visca"),
    ("6간담회실", "192.168.109.57", "http"),
    ("7간담회실", "192.168.110.57", "http"),
    ("8간담회실", "192.168.111.57", "visca"),
    ("9간담회실", "192.168.112.57", "http"),
    ("10간담회실", "192.168.113.57", "http"),
    ("11간담회실", "192.168.114.57", "http"),
]
...
def start_rtsp_stream(stream_req: StreamRequest):
    if stream_req.location not in STREAM_LOCATIONS:
    ...
    pid = play_rtsp(stream_req.location, stream_req.ip)
    if pid:
...
def start_rtmp_stream(stream_req: StreamRequest, background_tasks: BackgroundTasks):
    ...
    # 백그라운드 작업 시작
    background_tasks.add_task(_background_rtmp_task, stream_req.location, stream_req.ip, stream_req.stream_key)
...
def reset_device_endpoint(device: DeviceReset):
    if device.location not in RESET_IPS:
        raise HTTPException(status_code=400, detail="유효하지 않은 위치입니다.")
    
    ip = RESET_IPS[device.location]
